...
const assignNextTaskForGoal = async (userId, goalId) => {
  try {
    ...
    if (nextQueuedTask) {
      // Assign the task to today (Asia/Kolkata local time)
      const now = new Date();
      ...
      nextQueuedTask.assignedDate = todayLocal;
      nextQueuedTask.scheduledDate = todayLocal;
...
export const assignNextTaskManually = async (userId, goalId) => {
  try {
    ...
    if (incompleteTasks.length > 0) {
      throw new Error(
        "You must complete all your current tasks before requesting new ones"
      );
    }
