...
const hasAWSConfig = () => {
  try {
    ...
    console.log(
      "⚠️ AWS configuration not available, using local storage:",
      error.message
    );
    return false;
  }
...
// Configure storage based on AWS availability
let storage;
...
if (hasAWSConfig()) {
  ...
  storage = multerS3({
    ...
    key: (req, file, cb) => {
      ...
      const taskId = req.body.taskId || `temp-${Date.now()}`;
      const filename = generateUniqueFilename(file.originalname, taskId);
  ...
  storage = multer.diskStorage({
    destination: (req, file, cb) => {
      const submissionsDir = createUploadsDir();
      cb(null, submissionsDir);
    },
    filename: (req, file, cb) => {
      const taskId = req.body.taskId || `temp-${Date.now()}`;
      const filename = generateUniqueFilename(file.originalname, taskId);
      console.log("[upload middleware] Local upload configuration:", {
        originalName: file.originalname,
        generatedFilename: filename,
        taskId: taskId,
      });
...
const fileFilter = (req, file, cb) => {
  ...
  if (allowedTypes.includes(file.mimetype)) {
    ...
    cb(
      new Error(
        "Invalid file type. Only PDF, Excel, Word, and CSV files are allowed."
      ),
      false
    );
  }
...
// Configure multer with storage and enhanced security
const upload = multer({
  storage: storage,
  limits: {
    fileSize:
      process.env.NODE_ENV === "production"
        ? 5 * 1024 * 1024
        : 10 * 1024 * 1024, // 5MB in prod, 10MB in dev
