...
function generateAIPrompt(context) {
  ...
  return `You are an expert learning assistant. Generate a detailed, day-by-day real-world task plan for the entire learning timeline based on the following context:

CONTEXT:
- User Profile: ${JSON.stringify(context.userProfile)}
- Assessment Data: ${JSON.stringify(context.assessment)}
- Goal Setup: ${JSON.stringify(context.goal)}
- Roadmap Phases: ${JSON.stringify(context.roadmap)}
- Timeline (days): ${context.totalDays}
- Learning Path: ${context.learningPath}

REQUIREMENTS:
1. For EACH DAY in the timeline (from Day 1 to Day N), generate 1-3 practical, real-world tasks, distributed across the roadmap phases according to their duration.
2. Each task must be specific, actionable, and clearly aligned with ONE of the roadmap phases.
3. Include which phase the task belongs to using the numeric field "phase", referencing the provided roadmap phases.
4. Include the day number (1-based) for each task using the field "dayNumber".
5. Consider user's experience level and time commitment.
6. Include estimated duration (in hours).
7. Provide difficulty level (1-5).
8. Include learning resources and tips.
