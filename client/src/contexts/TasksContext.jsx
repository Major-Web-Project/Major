...
export function TasksProvider({ children }) {
  ...
  const normalizeTask = useCallback((task) => {
    ...
    return {
      ...
      category:
        taskData.category || task.category || taskData.type || "General",
  ...
  const getTasksByDate = useCallback(
    async (date) => {
      try {
        ...
        const normalizedTasks = rawTasks.map(normalizeTask);
        console.log(
          `[TasksContext] Tasks for ${dateString}:`,
  ...
  const submitTask = useCallback(
    async ({ taskId, submissionType, file, timeSpent }) => {
      try {
        setLoading(true);
        setError(null);
    ...
    try {
      ...
      if (response.success && response.data?.fileUrl) {
        ...
        if (
          ...
          console.log(
            "[TasksContext] Popup blocked, trying alternative approach"
          );
      ...
      if (error.response?.status === 404) {
        errorMessage =
          "Submission file not found. It may have been deleted or moved.";
      } else if (error.response?.status === 401) {
        errorMessage =
          "You are not authorized to view this file. Please log in again.";
      } else if (error.response?.status >= 500) {
        errorMessage =
          "Server error while retrieving the file. Please try again later.";
  ...
  const downloadTaskSubmission = useCallback(async (taskId, fileName) => {
    try {
      console.log("[TasksContext] downloadTaskSubmission called:", {
        taskId,
        fileName,
      });
