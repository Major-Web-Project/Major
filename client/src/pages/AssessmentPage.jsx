...
import { learningPaths, aiAssistant, assessmentQuestions } from '../services/aiLearningService';
...
const AssessmentPage = () => {
  ...
  const handleAssessmentComplete = (profile, responses) => {
    ...
    const assessmentData = {
      ...
      totalQuestions: Object.values(assessmentQuestions).reduce((sum, section) => sum + section.length, 0),
    ...
    localStorage.setItem('aiLearning_assessmentResponses', JSON.stringify(responses));
  ...
  const handleGoalSetupComplete = (goals, generatedRoadmap) => {
    setGoalData(goals);
    setRoadmap(generatedRoadmap);
    localStorage.setItem('aiLearning_goalData', JSON.stringify(goals));
    localStorage.setItem('aiLearning_roadmap', JSON.stringify(generatedRoadmap));
  ...
  const handleStartLearning = () => {
    ...
    localStorage.setItem('aiLearning_learningData', JSON.stringify(learningData));
    localStorage.removeItem('aiLearning_currentStep'); // Clear step as we're moving to dashboard
    navigate('/learning-dashboard');
  };
  ...
  const GoalSetupStep = () => {
    ...
    const handleSubmit = () => {
      ...
      const generatedRoadmap = aiAssistant.generatePersonalizedRoadmap(
        selectedPath,
        parseInt(timeframe),
        userProfile
      );

      handleGoalSetupComplete(goals, generatedRoadmap);
    };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {pathOptions.map((path) => (
                    <button
                      ...
                      className={`p-6 rounded-xl border-2 transition-all duration-300 text-left ${selectedPath === path.id
                        ? 'border-cyan-400 bg-cyan-500/20'
                        : 'border-white/20 bg-white/5 hover:border-white/40 hover:bg-white/10'
                        }`}
                    >
                      <div className="flex items-start gap-4">
                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl ${path.difficulty === 'advanced' ? 'bg-red-500/20' :
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-4 text-xs">
                            <span className={`px-2 py-1 rounded-full ${path.difficulty === 'advanced' ? 'bg-red-500/20 text-red-400' :
  ...
  const RoadmapStep = () => {
    ...
    }, [currentPhase, dayNumber]);

    const totalWeeks = roadmap?.phases.reduce((sum, phase) => sum + phase.duration, 0) || 0;
    const successProbability = Math.round((roadmap?.successPrediction || 0) * 100);
            ...
            <Card className="!bg-[#181D24] backdrop-blur-md border border-gray-700 rounded-3xl shadow-2xl mb-8">
              <CardContent className="p-8">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-center">
                    <div className={`text-6xl font-bold mb-2 ${successProbability >= 80 ? 'text-blue-500 dark:text-green-400' :
                ...
                <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="bg-gray-800/50 p-4 rounded-xl">
                    ...
                    <div className="text-white text-xl font-bold">{roadmap?.personalizedSchedule?.dailyHours || 0}h/day</div>
                  </div>
                  <div className="bg-gray-800/50 p-4 rounded-xl">
                    <div className="text-blue-400 font-semibold text-sm">Study Sessions</div>
                    <div className="text-white text-xl font-bold">{roadmap?.personalizedSchedule?.sessionsPerDay || 0}/day</div>
                  </div>
                </div>
              </CardContent>
            </Card>
                    ...
                    >
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-4">
                          <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl font-bold ${currentPhase === phase.phase
