...
class LocalFileManager {
  ...
  static cleanupOldFiles(options = {}) {
    ...
    try {
      ...
      if (cleanedCount > 0) {
        console.log(`🧹 Cleaned up ${cleanedCount} files, freed ${result.formattedFreedSpace}`);
      }
  ...
  static optimizeStorage(userId = null) {
    try {
      ...
      if (optimizedCount > 0) {
        console.log(`⚡ Optimized storage: removed ${optimizedCount} duplicates, saved ${result.formattedSpaceSaved}`);
      }
  ...
  static scheduleCleanup(options = {}) {
    ...
    console.log(`📅 Scheduled automatic cleanup every ${interval / (60 * 60 * 1000)} hours`);
  }
  ...
  static stopScheduledCleanup() {
    if (this._cleanupInterval) {
      ...
      console.log('⏹️ Stopped scheduled cleanup');
    }
  }
