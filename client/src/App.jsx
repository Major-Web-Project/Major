...
function App() {
  ...
  // Sync user state with authentication status from cookie on mount
  useEffect(() => {
    const checkAuthStatus = async () => {
      try {
        // Make a request to a protected endpoint to check for valid cookie
  ...
  const handleLogin = (userData) => {
    // No need to store token in localStorage, it's handled by http-only cookie
    setUser(userData);
    setIsAuthenticated(true);
  };
          ...
          <Route
            ...
            element={
              goalSet && learningData && roadmap && userProfile ? (
                ...
                <Navigate to="/learning-dashboard" replace />
              )
            }
          />
          <Route
