...
export const TaskSubmitButton = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Improved error handling: show backend reason and missing criteria if available
      const backend = error.response?.data;
      let errorMessage = "Please try again.";

      if (backend?.reason) {
        errorMessage = backend.reason;
        if (Array.isArray(backend.missingCriteria) && backend.missingCriteria.length > 0) {
          errorMessage += "\n\nMissing criteria:";
          backend.missingCriteria.forEach((c, i) => {
            errorMessage += `\n${i + 1}. ${c}`;
          });
        }
      ...
      } else if (error.message && error.message.includes("Task update failed")) {
