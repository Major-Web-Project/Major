...
export const TaskSubmitButton = ({
    ...
    const { submitTask, viewTaskSubmission, downloadTaskSubmission, loading } = useTasks();
    ...
    const handleSubmit = async () => {
        console.log('[TaskSubmitButton] handleSubmit called:', {
            submissionType,
        ...
        if (!submissionType || !selectedFile) {
            console.log('[TaskSubmitButton] Missing required fields');
        ...
        try {
            ...
            console.log('[TaskSubmitButton] Calling submitTask...');
            ...
            console.log('[TaskSubmitButton] submitTask completed successfully');
    ...
    const handleViewSubmission = async () => {
        try {
            console.log('[TaskSubmitButton] Viewing submission for task:', task.id);
            await viewTaskSubmission(task.id);
        } catch (error) {
            console.error('[TaskSubmitButton] Failed to view submission:', error);
            
            // Show user-friendly error message with download option
            const errorMessage = error.message || 'Failed to view submission file.';
            const shouldTryDownload = confirm(
                `Unable to open submission file: ${errorMessage}\n\nWould you like to try downloading the file instead?`
            );
            
            if (shouldTryDownload) {
                try {
                    const fileName = task.submissionFile ? 
                        task.submissionFile.split('/').pop() : 
                        `task-${task.id}-submission`;
                    await downloadTaskSubmission(task.id, fileName);
                } catch (downloadError) {
                    console.error('[TaskSubmitButton] Failed to download submission:', downloadError);
                    alert(`Download also failed: ${downloadError.message}`);
                }
            }
        }
    };
    ...
    const isCompleted = task.status === 'completed' && task.submissionFile;
    ...
    const shouldShowForm = showSubmissionForm && expandedTask === task.id && !isCompleted;
                ...
                <div className="w-full bg-gray-800/90 rounded-xl p-4 space-y-4 animate-fadeIn border border-gray-700/50">
                    ...
                    <div className="bg-cyan-500/10 rounded-lg p-3 border border-cyan-500/20">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-xl font-mono text-cyan-400">{formatTime(timeSpent)}</span>
                            </div>
                            <div className="flex gap-2">
                                {!isTimerRunning ? (
                                    <Button
                                        onClick={startTimer}
                                        className="bg-green-500 hover:bg-green-400 text-white px-3 py-1 text-sm rounded-lg"
                                    >
                                        ▶️ Start
                                    </Button>
                                ) : (
                                    <Button
                                        onClick={pauseTimer}
                                        className="bg-yellow-500 hover:bg-yellow-400 text-white px-3 py-1 text-sm rounded-lg"
                                    >
                                        ⏸️ Pause
                                    </Button>
                                )}
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <Button
                                ...
                                className={`p-3 border rounded-lg transition-all text-center ${submissionType === 'pdf'
                            ...
                            <Button
                                ...
                                className={`p-3 border rounded-lg transition-all text-center ${submissionType === 'excel'
                    ...
                    <div className="flex gap-2 pt-2 border-t border-gray-700/50">
                        <Button
                            ...
                            className="flex-1 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-400 hover:to-emerald-400 text-white py-2 font-medium rounded-lg disabled:opacity-50"
                        >
                            {submitting ? (
                                <>
                                    <span className="mr-2 animate-spin">⏳</span>
