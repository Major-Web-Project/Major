...
export const TaskReportSection = ({ tasks }) => {
    ...
    const getAdvancedTaskStats = () => {
        ...
        const totalActualTime = completed.reduce((sum, task) => sum + (task.actualTime || 0), 0);

        // Fixed efficiency calculation
        const efficiencyScores = completed.filter((task) => task.actualTime && task.actualTime > 0).map((task) => (task.estimatedTime / task.actualTime) * 100);
        const averageEfficiency = efficiencyScores.length > 0 ? efficiencyScores.reduce((sum, score) => sum + score, 0) / efficiencyScores.length : 0;

        const timeSaved = Math.max(0, totalEstimatedTime - totalActualTime);
        const productivityScore = completed.length > 0 ? (completed.length / tasks.length) * 100 : 0;
        ...
        const gradesReceived = completed.filter((task) => task.grade).map((task) => task.grade);
        const averageGrade = gradesReceived.length > 0 ? gradesReceived.reduce((sum, grade) => sum + grade, 0) / gradesReceived.length : 0;
    ...
    return (
        <div ref={container} className="bg-mint-100 border border-mint-200 rounded-3xl shadow-2xl dark:bg-white/10 dark:border-white/20 p-8 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-10">
                <div className="bg-gradient-to-r from-blue-500/20 to-cyan-500/20 p-6 rounded-2xl border border-blue-400/30 hover:scale-105 transition-transform duration-300">
                    ...
                    <div className="text-indigo-700 text-3xl font-bold mb-2 dark:text-white">{stats.total}</div>
                ...
                <div className="bg-gradient-to-r from-purple-500/20 to-pink-500/20 p-6 rounded-2xl border border-purple-400/30 hover:scale-105 transition-transform duration-300">
                    ...
                    <div className="text-indigo-700 text-3xl font-bold mb-2 dark:text-white">{stats.aiGenerated}</div>
                    <div className="text-purple-300 text-sm">{Math.round((stats.aiGenerated / Math.max(stats.total, 1)) * 100)}% of total</div>
                </div>
                <div className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 p-6 rounded-2xl border border-green-400/30 hover:scale-105 transition-transform duration-300">
                    <div className="flex items-center gap-3 mb-3">
                        <span className="text-3xl">✅</span>
                        <div className="text-green-400 font-bold text-lg">Completion Rate</div>
                    </div>
                    <div className="text-indigo-700 text-3xl font-bold mb-2 dark:text-white">{stats.completionRate}%</div>
                    <div className="text-green-300 text-sm">{stats.completed} of {stats.total} completed</div>
                </div>
                <div className="bg-gradient-to-r from-orange-500/20 to-red-500/20 p-6 rounded-2xl border border-orange-400/30 hover:scale-105 transition-transform duration-300">
                    <div className="flex items-center gap-3 mb-3">
                        <span className="text-3xl">⚡</span>
                        <div className="text-orange-400 font-bold text-lg">Efficiency Score</div>
                    </div>
                    <div className="text-indigo-700 text-3xl font-bold mb-2 dark:text-white">{stats.averageEfficiency}%</div>
                ...
                <div className="bg-gradient-to-r from-yellow-500/20 to-amber-500/20 p-6 rounded-2xl border border-yellow-400/30 hover:scale-105 transition-transform duration-300">
                    <div className="flex items-center gap-3 mb-3">
                        <span className="text-3xl">🎓</span>
                        <div className="text-yellow-400 font-bold text-lg">Average Grade</div>
                    </div>
                    <div className="text-indigo-700 text-3xl font-bold mb-2 dark:text-white">{stats.averageGrade}%</div>
            ...
            <div className="flex items-center justify-between bg-gradient-to-r from-indigo-500/20 to-purple-500/20 p-6 rounded-2xl border border-indigo-400/30">
                <div>
                    <h3 className="text-white font-bold text-xl mb-2">📊 Generate AI-Enhanced Performance Report</h3>
                    <p className="text-gray-300 text-sm">Export your detailed task performance analysis for {tasks.length} tasks</p>
                </div>
...
export const TasksScreen = ({ learningData, goalData, onTaskComplete }) => {
    ...
    useEffect(() => {
        ...
        // Fetch all tasks for the report section
        ...
        const handleTasksUpdated = async (event) => {
            ...
            console.log('TasksScreen: Tasks updated from other pages', event.detail);

            // Refresh all tasks when updates occur
            try {
                const response = await apiService.getTasks();
                if (!abortController.signal.aborted) {
                    setAllTasks(response.data?.tasks || []);
    ...
    const handleTaskCreated = async () => {
        ...
        // Refresh all tasks for the report section
        try {
            const response = await apiService.getTasks();
            setAllTasks(response.data?.tasks || []);
        } catch (error) {
