...
export const CalendarSection = ({ selectedDate, onDateSelect }) => {
                ...
                for (let day = 1; day <= daysInMonth; day++) {
                    ...
                    try {
                        ...
                        if (!abortController.signal.aborted) {
                            ...
                            const inProgressTasks = dayTasks.filter(t => t.status === 'in-progress').length;
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            days.push(
                <div
                    ...
                    className={`h-28 border border-sky-300/50 p-3 transition-all duration-300 relative overflow-hidden dark:border-gray-600/30 rounded-lg ${isFuture
                        ...
                        } ${hasData && !isFuture ? 'bg-gradient-to-br from-sky-200/50 to-sky-300/50 dark:from-gray-700/50 dark:to-gray-800/50' : !isFuture ? 'bg-sky-50/50 dark:bg-gray-800/20' : ''
                        }`}
                >
                    <div className={`text-sm font-bold mb-1 ${isFuture
                    ...
                    {hasData && (
                        <div className="space-y-1">
                            ...
                            <div className="flex items-center justify-between text-xs">
                                ...
                                <span className="text-lg">{getCompletionIcon(completionRate)}</span>
                            </div>
                            <div className={`text-xs font-bold text-center ${completionRate >= 70 ? 'text-green-400' :
    ...
    const maskStyle = !isDone && typed.length > 0 ? {
    ...
    return (
        <Card ref={container} className="bg-gray-900/50 border border-gray-700/50 rounded-3xl shadow-2xl dark:bg-white/10 dark:border-white/20">
                ...
                {!loading && (
                    <div className="bg-sky-100/50 rounded-xl p-6 border border-sky-200/50 mb-6 dark:bg-gray-800/60 dark:border-gray-700/50">
                        ...
                        <div className="grid grid-cols-7 gap-1 mb-3">
                            {dayNames.map(day => (
                                <div key={day} className="h-12 flex items-center justify-center text-sky-700 font-semibold text-base dark:text-gray-300">
                                    {day}
                                </div>
                            ))}
                        </div>
