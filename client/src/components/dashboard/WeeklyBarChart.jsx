...
export const WeeklyBarChart = ({ data, barsGridRef, allTasks = [] }) => {
  ...
  const calculateDailyProgress = (allTasks, currentDate = new Date()) => {
    ...
    const tasksPendingAtStartOfDay = safeTasks.filter(task => {
    ...
    const tasksCompletedToday = safeTasks.filter(task => {
    ...
    const totalGoal = Number(tasksPendingAtStartOfDay.length) || 0;
    const completedCount = Number(tasksCompletedToday.length) || 0;
  ...
  const dataToRender = Array.from({ length: 7 }).map((_, i) => {
    ...
    return {
      day: dayOfWeek,
      date: dateString,
      goal: 100, // Always 100% for visual consistency (blue bar always full height)
      completed: totalTasks > 0 ? Math.max(1, completionPercent) : 0, // Only show if there are tasks
      totalTasks,
      completedTasks,
      completionPercent,
                      ...
                      <div className="relative flex flex-col justify-end group h-full">
                        ...
                        >
                          <div className={`absolute top-2 bg-black/90 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap z-[9999] ${index <= 1
                            ...
                            Goal (Pending/In-progress): {item.totalTasks}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="relative w-8 min-w-[24px] h-full">
                      <div className="relative flex flex-col justify-end group h-full">
                        <div
                          ...
                          style={{
                            height: `${Math.max(1, item.completed)}%`,
                            borderTopLeftRadius: '20px',
                            borderTopRightRadius: '20px'
                          }}
                        >
                          <div className={`absolute top-2 bg-black/90 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap z-[9999] ${index <= 1
                            ...
                            Completed Tasks: {item.completedTasks}<br />Completion: {item.completed}%
                          </div>
                        </div>
                      </div>
                    </div>
