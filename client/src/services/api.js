...
console.log("[API] AI API instance created with NO timeout for unlimited task generation");
...
export const apiService = {
  ...
  getTasksByDate: async (date, goalId = null) => {
    try {
      ...
      console.log(`[API] Tasks for date ${date} (goalId: ${goalId}):`, tasks);
  ...
  getTasks: async (goalId = null) => {
    try {
      ...
      console.log(`[API] All tasks (goalId: ${goalId}):`, tasks);
  ...
  uploadTaskSubmission: async (formData) => {
    try {
      ...
      if (useIndexedDB) {
        try {
          console.log("[API] Using IndexedDB for large file:", file.size, "bytes");
        ...
        console.log("[API] Using localStorage for small file:", file.size, "bytes");
        ...
        try {
          ...
          if (localStorageError.name === 'QuotaExceededError') {
            ...
            console.log("[API] localStorage quota exceeded, trying IndexedDB...");
      ...
      console.log("[API] File stored successfully:", response);
      return response;
  ...
  viewTaskSubmission: async (filePath) => {
    try {
      ...
      try {
        ...
        console.log("[API] IndexedDB lookup failed, trying localStorage:", indexedDBError);
      }
  ...
  getFileMetadata: async (filePath) => {
    try {
      ...
      try {
        ...
        console.log("[API] IndexedDB metadata lookup failed, trying localStorage:", indexedDBError);
      }
...
export const createGoalWithUnlimitedTime = async (goalData) => {
  console.log(`[AI API] Creating goal with unlimited time for complete task generation`);
  console.log(`[AI API] Expected tasks for ${goalData.timeline || goalData.timeframe || 3} months: ~${(goalData.timeline || goalData.timeframe || 3) * 30} tasks`);
